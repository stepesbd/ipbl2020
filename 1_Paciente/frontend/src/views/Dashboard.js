...
export default function Dashboard (){
  ...
  const loadTotalPacientes = () => {
    setloadingT(true);
    let endPointComplete = "https://cors-anywhere.herokuapp.com/https://stepesbdmedrecords.herokuapp.com/api/positive/amount"
  ...
  const loadTotalRecuperados = () => {
    setloadingT(true);
    let endPointComplete = "https://cors-anywhere.herokuapp.com/https://stepesbdmedrecords.herokuapp.com/api/release/amount"
    UseGetApiURL(endPointComplete).then(result => {
      if (result.status !== 200) {
        setsalert(<SweetAlert warning title={result.message} onConfirm={hideAlert} />);
        setloadingT(false);
  ...
  const loadTotalObitos = () => {
    setloadingT(true);
    let endPointComplete = "https://cors-anywhere.herokuapp.com/https://stepesbdmedrecords.herokuapp.com/api/death/amount"
    UseGetApiURL(endPointComplete).then(result => {
      if (result.status !== 200) {
        setsalert(<SweetAlert warning title={result.message} onConfirm={hideAlert} />);
        setloadingT(false);
  ...
  <Container fluid className="main-content-container px-4">
    ...
    <Row noGutters className="page-header py-4">
      <PageTitle title="Indicadores" subtitle="Dashboard" className="text-sm-left mb-3" />
    </Row>
      ...
      {smallStats.map((stats, idx) => (
        <Col className="col-lg mb-4" key={idx} {...stats.attrs}>
          <SmallStats
            ...
            label={stats.label}
            value={idx === 0 ? totalPacientes :
                   idx === 1 ? totalRecuperados :
                   idx === 2 ? totalPacientes :
                   idx === 3 ? totalObitos :
                   stats.value}
