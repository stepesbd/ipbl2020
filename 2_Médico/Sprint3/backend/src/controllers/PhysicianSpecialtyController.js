...
module.exports = {
  async index(request, response) {
    const physicianSpecialties = await connection(
  ...
  async store(request, response) {
    ...
    return response
      .status(201)
      .json({ msg: { id, physicianId, specialtiesId } });
  },
  async update(request, response) {
    ...
    if (physician_specialty === 0) {
      return response
        .status(406)
        .json({ error: 'physician_specialty not updated!' });
    }
  ...
  async destroy(request, response) {
    ...
    if (physicianSpecialty === 0) {
      return response
        .status(406)
        .json({ error: 'physician_specialty not found!' });
    }
